...
| 사용 완료 이력 | 자주 사용하는 브랜드와 알림 우선순위 | 선택 초개인화 | 가격은 바꾸지 않고 동률 알림 우선순위만 조정 |
...
2. **표시 단계**: 절약액이 같은 후보만 만료 임박, 사용 선호, 원본 근거 신뢰도로 정렬한다.

사용 선호가 더 비싼 방법을 ‘최적’으로 표시해서는 안 된다. 금액 순위와 개인화된 표시 이유를 분리한다.
